...
export class ExamenService {
    ...
    private resourceUrl = 'api/examen';

    constructor(private http: Http, private dateUtils: DateUtils) { }

    create(examen: Examen): Observable<Examen> {
        let copy: Examen = Object.assign({}, examen);
        copy.date = this.dateUtils.toDate(examen.date);
        return this.http.post(this.resourceUrl, copy).map((res: Response) => {
            return res.json();
        });
    }

    update(examen: Examen): Observable<Examen> {
        let copy: Examen = Object.assign({}, examen);
    ...
    find(id: number): Observable<Examen> {
